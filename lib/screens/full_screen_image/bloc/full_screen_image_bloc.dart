...
class FullScreenImageBloc extends Bloc<FullScreenImageEvent, FullScreenImageState> {

  final List<String> imageReports;
  final ApprovedImage approvalState;
  final String url;

  FullScreenImageBloc(this.imageReports, this.approvalState, this.url) : super(FullScreenImageBaseState(shouldBlur(url, imageReports, approvalState))) {
    add(FullScreenImageInitialEvent());
  }

  @override
  Stream<FullScreenImageState> mapEventToState(FullScreenImageEvent event) async* {
    final currentState = state;
    if (event is FullScreenImageInitialEvent) {
    }else if(event is FullScreenImageUnblurEvent){
      yield FullScreenImageBaseState(false);
    }  else {
